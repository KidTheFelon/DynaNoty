...
4. **Добавьте тесты** (любым способом)
...
7. **Запушьте изменения**:
   ...
   ```
8. **Создайте Pull Request** с указанием метода разработки
...
4. **Запустите тесты** перед каждым коммитом
5. **Обновите документацию** при необходимости
...
```csharp
[Test]
public void ShowNotification_WithValidParameters_ShouldSucceed()
{
    // Arrange
    var logger = Mock.Of<ILogger<NotificationManager>>();
    var window = Mock.Of<INotificationWindow>();
    var manager = new NotificationManager(logger, window);

    // Act
    manager.ShowNotification("Test", "Subtitle", "🔔");

    // Assert
    Mock.Get(window).Verify(w => w.Show(), Times.Once);
}

[Test]
public void ShowNotification_WhenDisposed_ShouldThrowObjectDisposedException()
{
    // Arrange
    var manager = new NotificationManager(Mock.Of<ILogger<NotificationManager>>(), Mock.Of<INotificationWindow>());
    manager.Dispose();
...
# Конкретный проект
dotnet test Tests/DynaNoty.Tests.csproj
